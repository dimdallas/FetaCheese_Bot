...
public class MyBot_v8_timeManagement_1021 : IChessBot
{
    ...
    // Applied in previous version without iterative deepening
    // not enough tokens to add depthEvaluated property
    
    // For Iterative Deepening
    Move bestMoveThisTurn, bestMoveCurrIteration;
    int bestEvaluationCurrIteration;
    bool searchTimeout;
    // maybe add a HashSet with bestMoveCurrIteration found to check in move order
    // didn't help
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        // this way it makes good use of extra time while trying staying ahead in time and not spend it all
        var timeToThink = timeDifference > 0 ? 300 + timeDifference/2 : 300;   
        Console.WriteLine("Time to think " + timeToThink);  // #DEBUG
        ...
        for (var iterationDepth = 0; iterationDepth < maxValue; iterationDepth++)
        {
            ...
            if (bestMoveCurrIteration != Move.NullMove)
                // Because search starts by looking previous best move, so only gets possibly better
    ...
    int QuiescenceSearch( int alpha, int beta, Comparison<Move> orderingComparison, Board board) {
        ...
        // BIG DELTA Pruning, abandon, didn't increase performance
        // if greatest possible material swing not enough to raise alpha
        ...
        /*if (applyDeltaPruning && stubbornScore + 1700 < alpha) // stubbornScore  < alpha - bigDelta
            return alpha;*/
        ...
        foreach(var captureMove in captureMoves){
            ...
            if (applyDeltaPruning && stubbornScore + 200 + typeValues[(int)captureMove.CapturePieceType] < alpha)  // stubbornScore < alpha - delta
                continue;
    ...
    int MopUpEvaluation(bool whiteAdvantage, Board board)
    {
        ...
        // Would apply Manhattan Distance evaluation to Rooks and Queens
        // but not enough tokens, and many endgames do not include them, not until Pawn promotion
    ...
    // Manhattan Distance for Token efficiency
    ...
    // Manhattan Distance evaluation for specified piece
