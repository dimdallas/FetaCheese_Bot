...
public class MyBot_ab3_ordering_static_checks : IChessBot
{
    int movesChecked;
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        foreach (var legalMove in legalMoves)
        {
            ...
            var moveEvaluation = -AlphaBetaNegamaxSearch(minValue, maxValue, 3, board);
    ...
    int StaticEvaluation(Board board)
    {
        ...
        // var whitePieces = pieceLists.Take(6).ToArray();
        // var blackPieces = pieceLists.Skip(6).ToArray();
        ...
        var materialEvaluation = 0;
        
        for (var i = 0; i < 6; i++)
        {
            // materialEvaluation += (whitePieces[i].Count - blackPieces[i].Count) * typeValues[i + 1];
            materialEvaluation += (pieceLists[i].Count - pieceLists[i+6].Count) * typeValues[i + 1];
        }
