...
namespace ChessChallenge.Application
{
    public class ChallengeController
    {
        ...
        public void StartNewGame(PlayerType whiteType, PlayerType blackType)
        {
            ...
            //HERE IF LOAD A SPECIFIC FEN
            // fenIndex = 5;
            board.LoadPosition(botMatchStartFens[fenIndex]);
            // board.LoadPosition("8/5b2/8/8/3K2p1/6k1/8/7q w - - 20 123");  // played queen h1f3 eval 1340 and repetition move f3h1 eval 1340
            // board.LoadPosition("8/5b2/8/8/6p1/3K3k/8/3q4 w - - 30 128");  // played queen d1a4 eval 1340 and repetition move a4d1 eval 1340
            // board.LoadPosition("7N/2k5/8/8/1n6/5K2/8/8 w - - 0 1");
            // board.LoadPosition("7Q/2k5/1r6/6R1/1q6/3R1K2/8/8 b - - 0 1");
            // board.LoadPosition("r1b1k2r/pppp1Qpp/2n1p3/6p1/2q5/3N4/PP2R3/1K3R2 b kq - 1 24");
            // board.LoadPosition("rnb1kbnr/ppppqppp/8/8/8/3BPN2/PPP3PP/RNBQK2R b KQkq - 0 5");
            // board.LoadPosition("7k/6q1/6Q1/1r6/8/8/5PPP/R4R1K w - - 1 51");
            // board.LoadPosition("8/6k1/8/1r6/8/8/5PPP/R4R1K w - - 0 52");
            // Console.WriteLine("index " + fenIndex);
        ...
        ChessPlayer CreatePlayer(PlayerType type)
        {
            return type switch
            {
                PlayerType.MyBot => new ChessPlayer(new MyBot(), type, GameDurationMilliseconds),  // bot for bullet
                // PlayerType.MyBot => new ChessPlayer(new MyBot(), type/*, GameDurationMilliseconds*/),  // bot unlimited
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_negamax2_static(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_ab3_ordering_static_checks(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_ab3_ordering_static_trapKing(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_ab_qdelta_mat_trapking_ttEndgame(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v1(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v1_mobility(), type, GameDurationMilliseconds),
                PlayerType.EvilBot => new ChessPlayer(new MyBot_v2_endgameTT_abDepth2_deltaQsearch_matBal_minMob_pawnlessMopup(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v3_iterative(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v3_lessToken_betterEndgame(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v4_centerControl(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v5_partialMobility_980(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v5_noEarlyDepthBound_1099Tokens(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v6_betterAllocation_992(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v6_bestMovesSet(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v7_deltaPruning(), type, GameDurationMilliseconds),
                // PlayerType.EvilBot => new ChessPlayer(new MyBot_v8_timeManagement_1021(), type, GameDurationMilliseconds),
        ...
        void EndGame(GameResult result, bool log = true, bool autoStartNextBotMatch = true)
        {
            if (isPlaying)
            {
                ...
                if (log)
                {
                    ...
                    Log("Total ply number: " + board.plyCount, false, ConsoleColor.Green);
                }
                ...
                if (PlayerWhite.IsBot && PlayerBlack.IsBot)
                {
                    ...
                    // int numGamesToPlay = botMatchStartFens.Length / 5;
        ...
        public void Update()
        {
            if (isPlaying)
            {
                ...
                {
                    // Console.WriteLine(playMoveTime);
        ...
        // public int TotalGameCount => botMatchStartFens.Length / 5 ;
