...
public class MyBot_ab_qdelta_mat_trapking_ttEndgame : IChessBot
{
        ...
        foreach (var legalMove in legalMoves)
        {
            ...
            if (IsEndgame(board))
            {
                ...
                if (!transpositionTable.TryGetValue(board.ZobristKey, out moveEvaluation))
                {
                    ...
                    if (transpositionTable.TryAdd(board.ZobristKey, moveEvaluation))
                    {
                        // Console.WriteLine("Added Transposition " + board.ZobristKey);  // #DEBUG
                    }
                }
                else
                {
    ...
    int StaticEvaluation(Board board)
    {
        ...
        var materialEvaluation = GetMaterial(board, true) - GetMaterial(board, false);
        ...
        if (board.TrySkipTurn())
        {
            ...
            board.UndoSkipTurn();
            mobilityEvaluation = mobilityWeight * (whoPlays ? currPlayerMoves - nextPlayerMoves : nextPlayerMoves - currPlayerMoves);
        ...
        if (IsEndgame(board))
        {
            var trapEvaluation = TrapKingCaptureEvaluation(board);
    ...
    int TrapKingCaptureEvaluation(Board board)
    {
