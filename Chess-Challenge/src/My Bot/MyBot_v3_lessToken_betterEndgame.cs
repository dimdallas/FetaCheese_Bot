...
public class MyBot_v3_lessToken_betterEndgame : IChessBot
{
    ...
    int StaticEvaluation(Board board)
    {
        ...
        if (/*evaluation > 0 && */ /*IsEndgame(board) &&*/ board.TrySkipTurn())
        {
    ...
    int MopUpEvaluation(bool whiteAdvantage, Board board)
    {
        ...
        trapEvaluation += PieceSpecificMD(PieceType.Rook, opponentKingSquare, board, whiteAdvantage);
        trapEvaluation += PieceSpecificMD(PieceType.Queen, opponentKingSquare, board, whiteAdvantage);
    ...
    int PieceSpecificMD(PieceType type, Square enemyKingSquare, Board board, bool whiteAdvantage)
    {
        ...
        return myPieces.Sum(piece => ManhattanDistanceSubtraction(piece.Square, enemyKingSquare));
    }
